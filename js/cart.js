...
document.addEventListener("DOMContentLoaded", () => {
                        ...
                        <div class="p-2 bg-white mt-4 px-3 rounded">
                            ...
                            <div class="d-flex flex-row align-items-center justify-content-between">
                                <div class="d-flex flex-row align-items-center qty">
                                    <i class="fa fa-minus text-danger mr-2 minusQuantity" data-id="${item.id}" data-quantity="${item.quantity}"></i>
                                    <h5 class="text-grey mr-1 ml-1 quantityValue" data-id="${item.id}">${item.quantity}</h5>
                                    <i class="fa fa-plus text-success ml-2 addQuantity" data-id="${item.id}" data-quantity="${item.quantity}"></i>
                                </div>
                                <div>
                                    <h5 class="text-grey price"> &#8377; ${totalPrice} /-</h5>
                                </div>
                                <div class="d-flex align-items-center">
                                    <i class="fa fa-trash mb-1 text-danger delete-btn" data-id="${item.id}"></i>
                                </div>
                            </div>
                        </div>                    
                    </div>
    ...
    const handleQuantityChange = (itemId, change) => {
        const item = storeCartItems.find(item => item.id === itemId);
        if (!item) return;

        item.quantity += change;
        if (item.quantity <= 0) {
            storeCartItems = storeCartItems.filter(item => item.id !== itemId);
        }
    ...
    itemDiv.addEventListener("click", (e) => {
        ...
        if (target.classList.contains("addQuantity")) {
            handleQuantityChange(Number(target.getAttribute("data-id")), 1);
        }
        else if (target.classList.contains("minusQuantity")) {
            handleQuantityChange(Number(target.getAttribute("data-id")), -1);
        }
        else if (target.classList.contains("delete-btn")) {
            storeCartItems = storeCartItems.filter(item => item.id !== Number(target.getAttribute("data-id")));
            updateLocalStorage();
            renderCartItems();
        }
    });
